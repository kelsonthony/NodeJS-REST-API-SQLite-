...
class CarDao {
    ...
    /**
     * new find to get maker car
     */

    findByName(maker) {
        let sqlRequest = "SELECT maker, model, year FROM car WHERE maker=$maker";
        let sqlParams = {$maker: maker};
        return this.common.findOne(sqlRequest, sqlParams).then(row =>
            new Car(row.maker, row.model, row.year));
    };
